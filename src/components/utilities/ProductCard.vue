...
<template>
  <RouterLink :to="'/items/' + props.job?.id">
    <div class="card card-compact relative" @mouseover="isHovered = true" @mouseleave="isHovered = false">
      <figure>
        <div v-if="props.job?.old_price" class="badge bg-black text-white text-xs p-4 uppercase absolute top-0 right-0 z-20">
          Save Tk {{ props.job.old_price - props.job.price }}
        </div>
